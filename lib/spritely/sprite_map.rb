...
module Spritely
  class SpriteMap
    ...
    attr_reader :name, :environment, :options, :directory, :sort, :glob
    ...
    def inspect
      "#<Spritely::SpriteMap name=#{name} directory=#{directory} sort=#{sort} options=#{options}>"
    end
    ...
    def collection
      @collection ||= Collection.create(files, sort, options)
    end
